...
def initialize_database(): 
    ...
    # Initialize a new clean database so delete if already exists
    delete_database()
...
def test_database_connect(initialize_database_fixture):
    db = Singleton()
    db.get_cursor()
    assert 2 == len(db.sql("SELECT * FROM fish;"))
    #Close the connection, or test 4 fails
    db.connection.close()
...
def test_resetting_after_db_creation():
    ...
    #Close and delete database so database instance does not remain after tests
